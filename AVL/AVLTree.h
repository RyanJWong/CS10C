...
private:
   ...
   bool searchRecursive(const string &, Node *) const;
   Node *searchRecursiveNode(const string &, Node *) const;
   ...
   Node *getParent(Node *);
   Node *getParentRecursive(Node *, Node *);
...
public:
   ...
   void inOrder() const;
   int balanceFactor(Node *) const; // Return the balance factor of a given node.

   void printBalanceFactors() const; // Traverse and print the tree in inorder notation. Print the string followed by its balance factor in parentheses followed by a , and one space
};
